    ...
    votePollOption(pollData, callback) {
        async.waterfall([
            ...
            (connection, callback) => {
                ...
                // retrieve poll then use res to update poll
                rethinkdb.table('poll').get(pollData.id).run(connection, (err, res) => {
                    if (err) {
                    ...
                    rethinkdb.table('poll').get(pollData.id).update(res).run(connection, (error, result) => {
                        connection.close();
                        if (error) {
